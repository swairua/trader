        ...
        <div className="hidden lg:flex items-center space-x-12">
          {primaryLinks.map((item) => (
            <Link
              key={item.name}
              to={item.href}
              className={`text-sm font-medium hover:text-primary transition-none relative tracking-wide px-3 py-2 min-h-[44px] flex items-center ${
                isActive(item.href)
                  ? "text-primary"
                  : "text-foreground"
              }`}
              ...
              <DropdownMenu open={moreOpen} onOpenChange={(open) => setMoreOpen(open)}>
                ...
                >
                  <DropdownMenuTrigger asChild>
                    <button aria-haspopup="menu" className="text-sm font-medium px-3 py-2 min-h-[44px] flex items-center text-foreground pr-10 focus:outline-none focus:ring-0 hover:text-primary transition-none hover:bg-transparent" aria-expanded={moreOpen}>
                      <span className="block">{t('nav_more')}</span>
                      <span className="absolute right-2 top-1/2 -translate-y-1/2 h-6 w-6 flex items-center justify-center rounded-md bg-white/10 dark:bg-white/10 pointer-events-none">
